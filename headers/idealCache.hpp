...
private:
    ...
    std::multimap<std::size_t, KeyType>                 futureQueue_;

private:
    void cacheMiss(KeyType key, ValueType next) {
...
void IdealCache<KeyType, ValueType>::cacheRun() {
    ...
    auto eraseOne = [&](std::size_t when, const KeyType& k) {
    ...
    for (int i = static_cast<int>(values_.size()) - 1; i >= 0; --i) 
        nextOccurrences_[values_[static_cast<std::size_t>(i)]].push_back(static_cast<KeyType>(i));
